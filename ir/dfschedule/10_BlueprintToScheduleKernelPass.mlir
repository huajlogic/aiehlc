module attributes {codegen.headers = ["stdint.h", "stdio.h", "custom_lib.h"], routing.effective_k = 64 : i64, routing.full_k = 512 : i64, routing.k_rounds = 8 : i64, routing.pp_depth_map = {tensor_0 = 2 : i32, tensor_1 = 2 : i32, tensor_2 = 2 : i32}} {
  dfschedule.module @kernel_driver_dskernel_receiver {
    ...
    dfschedule.window_def @window_in_0 {acquire_lock = @LOCK_window_in_0_ACQ, async = true, buffer_size = 1024 : i32, direction = "in", num_rounds = 16 : i32, ping_buffer = @buf_in_ping_0, pong_buffer = @buf_in_pong_0, release_lock = @LOCK_window_in_0_REL}
    ...
    dfschedule.window_def @window_in_1 {acquire_lock = @LOCK_window_in_1_ACQ, async = true, buffer_size = 1024 : i32, direction = "in", num_rounds = 16 : i32, ping_buffer = @buf_in_ping_1, pong_buffer = @buf_in_pong_1, release_lock = @LOCK_window_in_1_REL}
    dfschedule.lock_def @LOCK_window_out_0_ACQ {id = 52 : i32, init_value = 0 : i32}
    dfschedule.lock_def @LOCK_window_out_0_REL {id = 53 : i32}
    dfschedule.buffer_def @buf_out_ping_0 : memref<1024xvector<4xi8>, "LOCAL"> {address = 507904 : i64}
    dfschedule.buffer_def @buf_out_pong_0 : memref<1024xvector<4xi8>, "LOCAL"> {address = 512000 : i64}
    dfschedule.window_def @window_out_0 {acquire_lock = @LOCK_window_out_0_ACQ, async = true, buffer_size = 1024 : i32, direction = "out", num_rounds = 4 : i32, ping_buffer = @buf_out_ping_0, pong_buffer = @buf_out_pong_0, release_lock = @LOCK_window_out_0_REL}
