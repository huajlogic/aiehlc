...
void dfschedulemanager::createdfschedulefuncByDim(OpBuilder& builder, MLIRContext* ctx,SymbolTable& symTable) {
        ...
        // 2) get tile handles
        auto shim_handle = builder.create<dfschedule::GetTileHandleOp>(location,
            builder.getI32IntegerAttr(2),
            builder.getI32IntegerAttr(0)
        ).getResult();

        auto core_A_handle = builder.create<dfschedule::GetTileHandleOp>(location,
            builder.getI32IntegerAttr(0),
            builder.getI32IntegerAttr(3)
        ).getResult();

        auto core_B_handle = builder.create<dfschedule::GetTileHandleOp>(location,
        ...
        auto streamA = builder.create<dfschedule::GetStreamHandleOp>(location, routeARef).getResult();
        auto streamB = builder.create<dfschedule::GetStreamHandleOp>(location, routeBRef).getResult();

        ///*
        // 4) launch DMA: %evt_dma = "dfschedule::LaunchKernelAsyncOp" on %shim_handle ( %gmem, %stream_A, %stream_B )
        ...
        auto pidA = builder.create<dfschedule::GetPacketIdOp>(location, routeARef).getResult();
        auto pidB = builder.create<dfschedule::GetPacketIdOp>(location, routeBRef).getResult();
        ...
        auto calleeAttrA = builder.getNamedAttr(dfschedule::LaunchKernelAsyncOp::getCalleeAttrName(), kernelARef);
        ...
        auto calleeAttrB = builder.getNamedAttr(dfschedule::LaunchKernelAsyncOp::getCalleeAttrName(), kernelBRef);
        ...
        // 7) wait on events
        builder.create<dfschedule::WaitOp>(location, ValueRange{evt_dma, evt_A, evt_B});
        //*/
...
mlir::func::FuncOp dfschedulemanager::createDSKernelFunc(OpBuilder &builder, MLIRContext *ctx) {
  ...
  // Lock Definitions:
  auto ping_acquire = builder.create<dfschedule::LockInitOp>(location, dfschedule::LockType::get(ctx), 0);
  auto pong_acquire = builder.create<dfschedule::LockInitOp>(location, dfschedule::LockType::get(ctx), 0);
  auto ping_release = builder.create<dfschedule::LockInitOp>(location, dfschedule::LockType::get(ctx), 1);
  auto pong_release = builder.create<dfschedule::LockInitOp>(location, dfschedule::LockType::get(ctx), 0);

  // Thread A (DMA): Launch DMA s2m loop - runs in parallel
  // NOTE: Ensure dfschedule_LaunchDmaS2MLoopOp is defined correctly in dfscheduleop.td
  ...
  auto ifAcquire = builder.create<mlir::scf::IfOp>(location, dfschedule::LockType::get(ctx), is_even, true);
  ...
  auto ifRelease = builder.create<mlir::scf::IfOp>(location, dfschedule::LockType::get(ctx), is_even, true);
