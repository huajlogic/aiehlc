...
def _section_mid(tile):
    if not ((_load_view() or {}).get("capabilities") or {}).get("ir", True):
        return ["(this app carries no dfschedule IR — the Info pane shows no "
                "IR tab for it; it is not generated by the aiehlc pipeline)"]
    ...
    if backend == "simulator":
        ...
        if sess and not sess.get("authorized"):
            note = ("Hardware backend is CONFIGURED but NO board session exists in "
                    ...
                    "\"Connect\", \"Run\", or \"Open Current Session\" first. "
...
def get_ui_state() -> str:
    ...
    The four named panes are AIE Debug (top-left, the array), Execution
    (bottom-left, app/board selection + run buttons + run log), Info
    (top-right, detail for the selection) and Tools (bottom-right, aiegdb /
    LLM / Search).
    """
...
_PANES = {
    ...
    "tile.hi":     "Info pane (top-right), tile Schedule tab — needs col,row",
    "tile.mid":    "Info pane, tile IR tab (dfschedule IR; aiehlc apps only, "
                   "no such tab for naiebaremetal) — needs col,row",
    "tile.lo":     "Info pane, tile Code tab (host.cc under aiehlc; kernel "
                   "source + .bcf + wrapper under naiebaremetal) — needs col,row",
...
Window panes (each is labelled with this name in the UI):
  ...
  Execution  bottom-left   app + board selection, Connect / Run /
                           Force stop, and the run log (see get_applog).
                           A single-app daemon has no app selector; a
                           --sim-only one has no board selector and no
                           Connect — Run is the only button
