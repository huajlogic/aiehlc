 ...
 *| ^	| ^		| ^	 | Note: Deprecated in Versal but supported in ZynqMP			     | | PM_GET_NODE_STATUS		|
 *0x3	| Both		| 2	 | V1 - The API is used to obtain information about current status of a device     | | ^
 ...
 *"type" first before performing the actual functionality				       | | PM_REGISTER_NOTIFIER	| 0x5	| Both
 *| 3	 | V1 - The API is used to register a subsystem to be notified about the\n device event
 *| | ^				| ^	| ^		| ^	 | V2 - Added support of event management functionality			     | | ^
 *| ^	| ^		| ^	 | Note: V2 is supported in Versal but ZynqMP supports only V1		     | | ^				| ^	| ^
 *| ^	 | V3 - Added support to unregister all notifiers at once		     | | PM_REQUEST_SUSPEND		| 0x6	| Both
 *| 1	 | The API is used to send suspend request to another subsystem		     | | PM_SELF_SUSPEND		| 0x7	|
 *Both		| 3	 | V1 - The API is used to suspend a child subsystem			     | | ^				| ^	| ^		| ^
 *| V2 - Added support of cpu idle functionality during force powerdown	     | | ^				| ^	| ^		| ^	 | V3 -
 *Added support of CPU off state					     | | ^				| ^	| ^		| ^	 | Note: V3 is supported in
 *Versal and Versal NET but ZynqMP supports only V1| | PM_FORCE_POWERDOWN		| 0x8	| Both		| 2	 | V1 - The API
 *is used to Powerdown other processor or node		     | | ^				| ^	| ^		| ^	 | V2 - Added support of cpu
 *idle functionality	during force powerdown	     | | ^				| ^	| ^		| ^	 | Note: V2 is supported in
 *Versal but ZynqMP supports only V1		     | | PM_ABORT_SUSPEND		| 0x9	| Both		| 1	 | The API is used
 *by a subsystem to abort suspend of a child subsystem	     | | PM_REQUEST_WAKEUP		| 0xA	| Both		| 1	 | The
 *API is used to start-up and wake-up a child subsystem		     | | PM_SET_WAKEUP_SOURCE	| 0xB	| Both		| 1	 |
 *The API is used to set wakeup source					     | | PM_SYSTEM_SHUTDOWN		| 0xC	| Both		| 1	 | The
 *API is used to shutdown or restart the system			     | | PM_REQUEST_NODE		| 0xD	| Both		| 2	 | V1 -
 *The API is used to request the usage of a device		     | | ^				| ^	| ^		| ^	 | V2 - Added support of
 *security policy handling during request device	     | | ^				| ^	| ^		| ^	 | Note: V2 is supported in
 *Versal but ZynqMP supports only V1		     | | PM_RELEASE_NODE		| 0xE	| Both		| 3	 | V1 - The API is
 *used to release the usage of a device		     | | ^				| ^	| ^		| ^	 | V2 - Added support of security
 *policy handling during request device      | | ^				| ^	| ^		| ^	 | Note: V2 is supported in Versal but
 *ZynqMP supports only V1		     | | ^				| ^	| ^		| ^	 | V3 - Added support for bulk device release
 *| | PM_SET_REQUIREMENT		| 0xF	| Both		| 1	 | The API is used to announce a change in requirement for a
 ...
 * | ^				| ^	| ^		| ^	 | Note: Deprecated in Versal but supported in ZynqMP			     |
 * | PM_INIT_FINALIZE		| 0x15	| Both		| 1	 | The API is used to initialize subsystem and release unused
 *devices	     | | PM_GET_CHIPID		| 0x18	| Both		| 1	 | The API is used to request the version and ID code of
 *a chip		     | | PM_PINCTRL_REQUEST		| 0x1C	| Both		| 1	 | The API is used to request the pin
 ...
 *| ^	 | Note: V2 is supported in ZynqMP but Versal supports only V1		     | | PM_IOCTL			| 0x22	| Both
 *| 3	 | V1 - The API is used to perform driver-like IOCTL functions on shared\n
 *system devices									       | | ^				| ^	| ^		| ^	 | V2 - Added support of
 *bitmask functionality, user can check the supported\n									ID first before performing the
 *actual functionality				       | | ^				| ^	| ^		| ^	 | V3 - Add support of zeroization of
 *AIE data and program memory separately | | ^				| ^	| ^		| ^	 | Note: V3 is supported in Versal but
 *ZynqMP supports only V2	             | | PM_QUERY_DATA		| 0x23	| Both		| 2	 | V1 - The API is used to query
 *information about the platform resources  | | ^				| ^	| ^		| ^	 | V2 - Added support of bitmask
 *functionality, user can check the supported\n									ID first before performing the actual
 *functionality				       | | PM_CLOCK_ENABLE		| 0x24	| Both		| 1	 | The API is used to enable the
 *clock					     | | PM_CLOCK_DISABLE		| 0x25	| Both		| 1	 | The API is used to disable the clock
 ...
 *| ^	| ^		| ^	 | Note: Deprecated in Versal but supported in ZynqMP			     | | PM_EFUSE_ACCESS		|
 *0x35	| ZynqMP	| 1	 | The API is used to provide access to efuse memory			     | | ^				| ^	| ^
 *| ^	 | Note: Deprecated in Versal but supported in ZynqMP			     | | PM_FEATURE_CHECK		| 0x3F	| Both
 *| 2	 | V1 - The API is used to return supported version of the given API	     | | ^				| ^	| ^		| ^
 ...
 *
 * This section provides the details of the IOCTL IDs which are supported across the different platforms and their brief
 *descriptions.
 *
 ...
 *
 * This section provides the details of the QUERY IDs which are supported across the different platforms and their brief
 *descriptions.
 *
 * | Name					| ID	| Platform	| Description				|
 * |--------------------------------------------|-------|---------------|---------------------------------------|
