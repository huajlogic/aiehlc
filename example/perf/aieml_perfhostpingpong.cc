...
__global__ void perf(input_window_int32 * win ,output_window_int32 *out) {
...
#define VECTOR_LENGTH 16
	...
	uint32_t * ptr_in2 = (uint32_t *)(0x70000 + 0x2000);
	uint32_t * ptr_array[2] = {ptr_in, ptr_in2};
	uint32_t * selected_ptr = (uint32_t *)(0x70000 + 0x7000);

	uint32_t * ptr_in2_now = ptr_array[*selected_ptr];
	
	uint32_t * vec1 = ((uint32_t*)ptr_in2_now), * vec2 = ((uint32_t*)ptr_in2_now + MAT_SIZE);
...
int test_routing(XAie_DevInst *DevInst)
{
    ...
    for(int i = 0; i < 5; i++) {
		for(int j = 0; j < mlen; j++) {
			((int32_t*)vmem)[i] = i + 1 + j;
			((int32_t*)vmem_out)[i] = j;
		}
