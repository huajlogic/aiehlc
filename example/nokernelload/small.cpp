...
__kernel__ void k(int *in __attribute__((annotate("mem_address:0x1000"))),
                  int *out __attribute__((annotate("mem_address:0x2000")))) {
    for (int i = 0; i < 20; ++i)
        *(out++) = 100 + i;
}
...
int test_kernel(XAie_DevInst *DevInst) {
    ...
    if (mismatches == 0) {
        printf("Success: CPU result matches AIE.\n");
        printf("\n\nDone with kernel 1!\n\n");
        return 0;
    } else {
