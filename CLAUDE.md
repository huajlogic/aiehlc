...
`DfscheduleKernelAggregationPass` (`pass/passdfschedulekernelaggregation/`, gated on `routing.kernel_config_offload`) collapses the **per-core-tile** core DMA config into one BD group per window. `BlueprintToScheduleKernelPass` emits a `declaretile`/`dma_bd`/`create_io`/`start_io` group per tile — 48 groups / 96 `dma_bd` on a 4×4 mesh with 3 windows — but one `kernel.cc` is broadcast to every core tile, so those describe at most two distinct configurations: S2MM is identical on every tile (circuit-switched: no packet id, no ooo bd), and MM2S differs only in `packet_id` and `ooo_bd_id`. The pass rewrites each cluster onto **`dfschedule.declaretile.self`** — the tile executing this kernel, resolved on-core via `get_coreid()` — carrying the fan-out as discardable attrs `aggregated` / `tile_coords` / `tile_packet_ids` / `tile_ooo_bd_ids` (96 → 6 `dma_bd`, IR 2002 → 247 lines). One self-tile is created at the top of the kernel module body so it dominates every group, and the now-dead per-tile `declaretile`s are swept with the redundant BDs.

`declaretile.self` is a distinct **op**, not a `declaretile {col = -1, row = -1}` sentinel, so that "has no coordinates" is a *type-level* fact: all ~18 `getDefiningOp<DeclareTileOp>()` sites return null for it and fail closed rather than silently reading `-1`. (The sentinel was also easy to test wrongly — `DeclareTileOp::getCol()` returns `uint32_t`, so `getCol() < 0` is always false while `getCol() == -1` works.) It produces the same `!dfschedule.tile`, so the four kernel-path consumers — `config.dma_bd`, `config.create_io`, `schedule.getbdid`, `bind_core_buffer` — accept it unchanged; `load_kernel_group` is host-only. **It is kernel-path only:** `DfscheduleToApiPass` walks for it up front and fails the build with an explicit message, because the host lowers a tile handle to `XAie_TileLoc(col,row)` and a self-tile would otherwise become a bogus address.

**`packet_id` / `out_of_order_bd_id` are SSA operands, not attributes** (`td/dfscheduleop.td`), sitting alongside `bd_id` and `offset`: `dfschedule.config.dma_bd(%buf, %tile, %bd_id, %offset, %packet_id, %ooo_bd_id [, %linked_bd])`. `linked_bd` stays **last** in the operand list because it is the only optional one, which is what lets the hand-written parser in `dfschedulemanager.cpp` disambiguate it positionally; the generated `getODSOperandIndexAndLength` derives its variadic offset from the non-optional count, so no `AttrSizedOperandSegments` is needed. They must remain `arith.constant`-foldable — the kernel path bakes them into `kernel.cc` and has no runtime path for them, so `DfscheduleKernelAggregationPass` **hard-fails** on a non-constant (same contract as `bd_id`), while the host provenance comment degrades to `-1`, the provenance JSON to `"runtime"`, and `ScheduleCanonicalizePass` simply never folds. **Watch out:** that pass's merge-legality check compares the *attribute dictionary*; these two fields leaving it would make a per-tile difference invisible rather than merely unchecked, so the check also compares operands, skipping exactly indices 4 and 5 (`isPerTileVaryingOperand`).
...
- **`include/bspcompat/`** — shims for standalone-BSP headers that exist only on the armclang branch. `thirdparty/alib/include/` is a flattened copy of *one* BSP (picked by `--aie-version`, wiped and re-copied every run, gitignored), and the Gen2 `psv_cortexa72_0` BSP omits `xpseudo_asm_armclang.h` — which `xpseudo_asm.h` selects under `#ifdef __clang__`, i.e. exactly during the aiehlc front-end parse (the host object is built by `aarch64-none-elf-g++`, which takes the `_gcc.h` branch). `aiehlc.sh` appends `-I${AIEHLC_DIR}/include/bspcompat` **last** in `AIEHLC_ARGS`, so the real Gen5 header still wins; add it to the front-end only, never to the host/kernel compiles. Related: `aie_runtime.c`'s XTime header gate is `AIE_GEN > 2`, not bare `__has_include("xiltimer.h")` — the a72 BSP ships that header unconfigured, so its `COUNTS_PER_SECOND` expands to an undefined `XPAR_CPU_TIMESTAMP_CLK_FREQ`. (skill: bspheadergen2)
